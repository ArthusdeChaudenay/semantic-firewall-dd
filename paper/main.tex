...
% Broken by hand: left to itself the title puts "the Method" alone on the second line.
\title{Auditing Scaffolding Gains in LLM Financial Extraction \\ When the Label Is the
Method}
...
That promise is why we built an identity-based corrector. On 104 annual reports, scored
against reference values taken from regulatory tags, it appeared to improve substantially
over zero-shot extraction at no additional inference cost. The apparent gain came from one
field. EBITDA has no regulatory tag, so the evaluation computed its reference value as
operating income plus depreciation and amortisation, and the corrector computed the field
by the same formula. The score on it therefore tracks the two
constituent fields rather than the rule under test, and it accounts for the entire
improvement. Rescored without it, six configurations become statistically
indistinguishable and four produce identical values.

We call this a method-coupled label, one produced by a rule the system under test also
applies. We give a rescoring test that costs no computation, estimate the inflation from
the coupling rule and the raw extraction, and reproduce the effect by injecting couplings
deliberately. The inflation is largest where the constituent fields
are already accurate, so it flatters a mature pipeline more than a weak one. What
survives is that zero-shot extraction is strong on this task and that none of the
scaffolding we tested improves it.
\end{abstract}
...
knowable without a reference value for any of them. The same identities can then
repair what they indict. The design is a natural one, close in spirit to checking generated code against
executable tests \citep{chen2023codet}. This paper reports what happened when we built
it and then audited the result.\footnote{Code, extractions, reference values and every
script that produces a number in this paper:
\url{https://github.com/ArthusdeChaudenay/semantic-firewall-dd}.}

At first the system looked successful. Scored against reference values drawn from
regulatory tags, the corrector improved field accuracy over zero-shot extraction at no
additional inference cost, and exceeded chain-of-thought self-verification at half the
model calls. The improvement, however, came entirely from a circularity in the
evaluation. EBITDA has no regulatory tag, so its label was derived from operating income
and depreciation and amortisation, and the corrector reconstructed the same quantity by
the same identity. The evaluation counts one success a second time under a third name.
...
that the system under test also applies is method-coupled. Performance on such a field
does not independently test the corresponding system rule, because correctness is mechanically
tied to the correctness of the constituent fields. A gain attributed to it measures
those fields, not the rule.
\end{quote}
\end{samepage}

Written down, the condition is close to definitional, a form of target leakage in domain
clothing. The interesting question is not whether it is true but why it is missed.
Evaluation
...
\citep{sainz2023nlp,recht2019imagenet}. This one is quieter. No example leaks from train to
test and no shortcut lets a model bypass the input, so a held-out split and a
contamination check both pass; a missing reference label is simply filled in by a rule
that later appears inside the system. In our case the coupling was
...
What we add to the definition is a test, a predictor, and a measurement of severity. We
audited only our own system and offer no evidence on prevalence.
...
The predictor has two inputs. The label definition supplies the form of the rule, and the
raw extraction supplies the documents where the derived field was omitted and the
constituent fields were already correct. The magnitude is therefore a property of the
coupling and of the system's constituent-field accuracy together, and Section
\ref{sec:inject} measures that dependence directly.
...
reporting structure far more than configurations differ from one another, a
corpus-level number is largely a statement about sector composition rather than a
measure of how well extraction would work on a differently balanced corpus. The
comparison between
...
Four measurement pitfalls surfaced in the course of this work. Appendix
\ref{app:traps} states each as a mechanism, the instrument that detects it, and the
...
\paragraph{Recommendations.} Begin by marking which reference values are read from the
source and which are computed, and compare the computed ones against the rules of the
system being scored. Verify over quantities the filer is required to disclose,
...
The check is cheap: whenever a missing reference value is computed, ask whether the system under
test computes it too.
...
% Likewise the appendices, which otherwise started partway down the second page of
% references.
\clearpage
\appendix
...
% A table* is never set on the page where it is read, so this one is declared
% two appendices ahead of the Appendix F text that discusses it in order to
% reach the top of that page rather than a page of its own.
\begin{table*}[t]
\centering\small
...
A field's reference value is produced by a formula the system also applies, so the field
does not independently test that rule &
Rescore with such fields excluded; estimate the inflation from the coupling rule and the
baseline extraction outputs (\S\ref{sec:predict}) &
...
model access. The repository is
\url{https://github.com/ArthusdeChaudenay/semantic-firewall-dd}.
